...
#define PORT 12810 // server port
#define LED_PORT 12820 // server port
...
static const char short_options [] = "d:h:q:mruW:H:I:vc";
...
{	
	...
	int serv_addrlen = sizeof(serv_address);

	char buffer[BUFFER_SIZE];
	int cnt_img = 0;
	char cnt_img_str[100];
	...
	// Creation of the socket file descriptor 
    	serveur_fd = socket(AF_INET, SOCK_STREAM, 0); // TCP
    	if (serveur_fd == SOCKET_ERROR) {
        	perror("socket failed");
        	exit(EXIT_FAILURE);
    	}

	// Creation of the socket file descriptor (led)
    	led_fd = socket(AF_INET, SOCK_STREAM, 0); // TCP
    	if (led_fd == SOCKET_ERROR) {
        	perror("socket failed");
        	exit(EXIT_FAILURE);
    	}
	...
	// Additional option for the definition of the socket
    	if (setsockopt(serveur_fd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt, sizeof(opt)) == SOCKET_ERROR) {
        	perror("setsockopt failed");
        	exit(EXIT_FAILURE);
    	}

	fprintf(stderr, "Socket options successfully defined..\n");
	...
	serv_address.sin_addr.s_addr = inet_addr("127.0.0.1");
    	serv_address.sin_port = htons(LED_PORT); 


	if (connect(led_fd, (struct sockaddr *)&serv_address, serv_addrlen) == SOCKET_ERROR) 
	{ 
	...
	// Binding the socket to the address and port number
    	if (bind(serveur_fd, (struct sockaddr *)&address, addrlen) == SOCKET_ERROR) {
        	perror("bind failed");
        	exit(EXIT_FAILURE);
    	}
	...
	// Now server is ready to listen and verification
    	if ((listen(serveur_fd, 3)) == SOCKET_ERROR) {
	...
	// Accepting a connection from a client
    	if ((serveur_com = accept(serveur_fd, (struct sockaddr *)&address,(socklen_t*)&addrlen)) == SOCKET_ERROR) {
        	perror("accept failed");
        	exit(EXIT_FAILURE);
   	}

	fprintf(stderr, "Connection accepted with a client..\n");

	if (read(serveur_com, buffer, sizeof(buffer)) == SOCKET_ERROR) {
        	perror("read failed");
        	exit(EXIT_FAILURE);
   	}


	while (buffer[0] != 'q') {  // while the client does not send end request

		if (buffer[0] == 't') {
			
			send(led_fd, "on", strlen("on"), 0);
			sprintf(cnt_img_str ,"%d", cnt_img); 
			jpegFilename = strcat(cnt_img_str, "camera.jpg");
			fprintf(stderr, "Taking the camera image %s...\n", jpegFilename);
			...
			deviceClose();
    			...
    			if ((img = fopen(jpegFilename, "r")) == NULL) {
				perror("fopen failed");
        			exit(EXIT_FAILURE);
			}

			fprintf(stderr, "Estimating the size of the image %s...\n", jpegFilename);
    			fseek(img, 0, SEEK_END);
    			size_img = ftell(img); // size of the image taken
    			fseek(img, 0, SEEK_SET); // put the index at the beginning of the file
			char img_buffer[size_img];
				

			fprintf(stderr, "Sending the image %s...\n", jpegFilename);
      			if (fread(img_buffer, sizeof(char), size_img, img)) {
				int32_t size_total_buffer = sizeof(img_buffer);
				fprintf(stderr, "%d\n", size_total_buffer);
				size_total_buffer=htonl(size_total_buffer);
				send(serveur_com, (const char *)&size_total_buffer, 4, 0);
        			send(serveur_com, &img_buffer, sizeof(img_buffer), 0);

			}
    			
    			fprintf(stderr, "Image %s sent\n", jpegFilename);
    			cnt_img += 1;
			...
			send(led_fd, "off", strlen("off"), 0);


		}

		else {
      			fprintf(stderr, "Keeping the connection alive... \n");
      			send(serveur_com, "ok", strlen("ok"), 0);
    		}
